...
def plot_network_output(scores, board, history, out_directory, output_file,
    ...
    if western_column_notation:
        plt.xticks(list(range(1, board.size + 1)), list(range(1, board.size + 1)))
        plt.yticks(list(range(1, board.size + 1)), reversed(list(range(1, board.size + 1))))
    # Traditional has the origin at the upper-left and uses letters minus 'I' along the top
    else:
        ax.xaxis.tick_top()
        plt.xticks(list(range(1, board.size + 1)), [x for x in LETTERS[:board.size + 1] if x != 'I'])
        plt.yticks(list(range(1, board.size + 1)), list(range(1, board.size + 1)))
